...
def ConstructGraph(gyri_network, sulci_network,unknown_network,gyri_signal,sulci_signal, gyri_pos, sulci_pos, roi, mask_label):
    matrix = np.zeros((35559,35559), dtype='i8' )  #first 17232 gyri, 18327 sulci
...
def main(path, pkl_name, save_path, id_part, tr_type,THRESHOLD):
    ...
    matrix = np.zeros((35559,35559),dtype='i8')
...
for THRESHOLD in THRESHOLDs:
    ...
    for pkl in pkls:
        ...
        if(pkl == 'gyri_weight.pkl'):   
            ...
            #subject_signal = subject_signal/ subject_signal.max()
            scalars = []
            labels = []
            #write this signal 
            ...
            rewrite_scalars("/media/shawey/SSD8T/GyraSulci_Motor/H5_vtk/100408.vtk", \
                    save_path+'/'+sub_id+"_"+part+'_'+str(THRESHOLD)+"_zscore.vtk",new_scalars=scalars,new_scalar_names=labels)
